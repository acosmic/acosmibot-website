// Developer experiment only: no import from the shipped Activity entry point.
import { Application, CanvasSource, Color, Geometry, Mesh, Shader, Sprite, Texture } from 'pixi.js';
import { drawBlackHole } from '../../src/activities/event-horizon/black-hole.mjs';
import { holeGeometry } from '../../src/activities/event-horizon/render-cache.mjs';
...
function arcMesh(arcs, camera, width, height, ratio) {
  ...
  const shader=Shader.from({gl:{vertex,fragment},resources:{scene:{
    uViewport:{value:new Float32Array([width,height]),type:'vec2<f32>'},
...
export async function createPixiHole({canvas,width,height,ratio=2,camera,reduced=false}){
  const app=new Application();
  await app.init({canvas,width,height,resolution:ratio,autoDensity:true,antialias:true,
  ...
  function staticLayer(name,extent){
    ...
    const sprite=new Sprite(texture);sprite.position.set(left,top);app.stage.addChild(sprite);
  }
  function arcs(data){const item=arcMesh(data,camera,width,height,ratio);meshes.push(item);app.stage.addChild(item.mesh);}
  ...
  if(!reduced)for(let i=0;i<60;i++){
    ...
    sprite.width=sprite.height=.8+i%2;app.stage.addChild(sprite);embers.push(sprite);
  }
  ...
  render(0); // Allocate/upload outside measured steady-state frames.
  return {app,render,textureBytes,destroy(){
    ...
    app.destroy({removeView:false,releaseGlobalResources:true});
  }};
}
