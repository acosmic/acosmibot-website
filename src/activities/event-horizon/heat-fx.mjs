...
export function drawNoseHeat(ctx, size, heat, time, reduced = false) {
  ...
  const width = size * (.055 + .085 * level);
  const tip = size * .035;
  const glow = ctx.createLinearGradient(-length, 0, tip, 0);
  glow.addColorStop(0, 'rgba(255,0,0,0)');
  glow.addColorStop(.45, `rgba(255,15,8,${.12 * intensity})`);
  glow.addColorStop(.85, `rgba(255,35,15,${.4 * intensity})`);
  glow.addColorStop(1, `rgba(255,95,45,${.7 * intensity})`);
  ctx.fillStyle = glow;
