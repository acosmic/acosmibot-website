...
export const SlotsSection: React.FC<SlotsSectionProps> = ({ value, onChange, availableEmojis }) => {
  ...
  const allUsed = useMemo(
    () => TIERS.flatMap(t => value.tier_emojis[t.tier] ?? []),
    [value],
  );

  const openPickerAdd = (tier: SlotsTier) => { setPickerTier(tier); setReplaceIndex(null); };
  const openPickerReplace = (tier: SlotsTier, index: number) => { setPickerTier(tier); setReplaceIndex(index); };
  const closePicker = () => { setPickerTier(null); setReplaceIndex(null); };

  const handleSelectEmoji = (emoji: string) => {
    if (!pickerTier) return;
    const current = [...(value.tier_emojis[pickerTier] ?? [])];
    if (replaceIndex !== null) current[replaceIndex] = emoji;
    else current.push(emoji);
    onChange({ tier_emojis: { ...value.tier_emojis, [pickerTier]: current } });
    closePicker();
  };

  const removeEmoji = (tier: SlotsTier, index: number) => {
    const current = [...(value.tier_emojis[tier] ?? [])];
    current.splice(index, 1);
    onChange({ tier_emojis: { ...value.tier_emojis, [tier]: current } });
  };
  ...
  return (
    <>
      ...
      {TIERS.map(meta => {
        const emojis = value.tier_emojis[meta.tier] ?? [];
        const count = emojis.length;
        const full = count >= meta.limit;
        const sectionDisabled = !value.enabled;
        return (
