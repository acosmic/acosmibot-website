  ...
  return (
    <div className="form-group mb-3">
      ...
      <div
        ...
        style={{ height: 'auto', minHeight: '42px', display: 'flex', flexWrap: 'wrap', gap: '8px', padding: '8px' }}
      >
        {value.length === 0 && (
        ...
        {value.map(id => {
          ...
          return (
            ...
            >
              #{ch?.name || id}
              <span style={{ cursor: 'pointer', fontWeight: 'bold' }} onClick={() => toggle(id)}>×</span>
            </span>
          );
        })}
      </div>
      {channels && channels.length > 0 && (
        <div
          ...
          style={{ maxHeight: '200px', overflowY: 'auto', background: 'var(--bg-tertiary)', border: '1px solid var(--border-light)' }}
        >
          {channels.map(ch => (
            <div
              key={ch.id}
