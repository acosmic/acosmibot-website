...
async function updateNavForLoggedIn() {
    ...
    loginBtn.innerHTML = `
        <div class="user-avatar-nav" style="background-image: url('${currentUser.avatar}');"></div>
    `;
...
function setupMobileNav() {
    ...
    // Inject hamburger button before the login button
    const hamburger = document.createElement('button');
    hamburger.className = 'hamburger-btn';
    hamburger.id = 'hamburgerBtn';
    hamburger.setAttribute('aria-label', 'Open navigation menu');
    hamburger.innerHTML = '<span></span><span></span><span></span>';
    navContent.insertBefore(hamburger, navContent.lastElementChild);
    ...
    const existingLogoImg = document.querySelector('.logo img');
    const logoSrc = existingLogoImg ? existingLogoImg.src : '/images/acosmibot_website-logo.png';

    drawer.innerHTML = `
        <div class="mobile-nav-header">
            <a href="/" class="mobile-nav-logo">
                <img src="${logoSrc}" alt="Acosmibot">
            </a>
    ...
    // Sync auth state now that drawer exists
    updateMobileDrawerAuth(!!currentUser);
}
...
function updateMobileDrawerAuth(isLoggedIn) {
    ...
    if (mobileLoginBtn && isLoggedIn && currentUser) {
        mobileLoginBtn.innerHTML = `
            <div class="user-avatar-nav" style="background-image: url('${currentUser.avatar}');"></div>
            <span style="font-size:0.9rem; font-weight:600; color: var(--text-primary);">${currentUser.username}</span>
        `;
        mobileLoginBtn.style.background = 'var(--bg-overlay)';
        mobileLoginBtn.style.border = '1px solid var(--border-light)';
        mobileLoginBtn.style.borderRadius = '12px';
        mobileLoginBtn.style.padding = '10px 16px';
    }
}
